...
--   2) 黄金有好几个独立品种：XAUUSD.c（美分账户）、XAUUSDs（标准账户）、
--      XAUUSD —— 后缀不同就是不同合约，点差、波动、最小手数都不一样，绝不可
--      加在一起比较。按品种统计时它们会自动分开（GROUP BY symbol 即可），
--      但判断"这个策略行不行"时必须只看同一个品种。
--   3) 平均盈亏会被【个别大手数账户】带偏（曾有一笔 3 手黄金 -4197，把 5 单的
--      小时平均拉到 -804）。所以【胜率】和【每 0.01 手盈亏】才是主口径，
--      平均盈亏只在同一个客户内看才有意义。
--   4) 别名里带点号必须加反引号（`每0.01手盈亏`），否则 MySQL 报 #1064。
