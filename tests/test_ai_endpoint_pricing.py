...
def test_vision_answer_accepts_equivalent_english_and_chinese_shape_names() -> None:
    ...
    assert _vision_test_answer_is_correct("RED SEMICIRCLE, BLUE SQUARE") is True
    assert _vision_test_answer_is_correct("RED SEMICIRCLE, BLUE SQUARES") is True
    ...
    assert _vision_test_answer_is_correct("BLUE SEMICIRCLE, RED SQUARE") is False
...
def test_vision_parameter_rejection_retries_new_and_minimal_parameters(tmp_path: Path, monkeypatch) -> None:
    ...
    assert request_bodies[0]["max_tokens"] == 1024
    assert request_bodies[0]["temperature"] == 0
    assert request_bodies[1]["max_completion_tokens"] == 1024
